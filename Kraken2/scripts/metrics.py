...
def read_alpha(path):
    """Parse Shannon alpha diversity from KrakenTools output."""
    ...
    # KrakenTools versions can differ in their label text. If no label is
    # present, use the last numeric value printed by the command.
    numbers = re.findall(
        r"[-+]?(?:\d+\.\d+|\d+|\.\d+)(?:[eE][-+]?\d+)?",
        text,
    )
...
krona_by_sample = {
    sample_from_path(path, ".html"): path
    for path in snakemake.input.krona
}
...
for sample in samples:
    ...
    row = {
        ...
        "krona_html": os.path.abspath(krona_by_sample[sample]),
    }
