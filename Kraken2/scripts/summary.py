...
# Ingest and structure Bracken taxonomy tables per sample
bracken_data = {}
for path_str in snakemake.input.bracken:
    path = Path(path_str)
    sample_name = path.name[: -len(".bracken")]
    try:
        df_b = pd.read_csv(path, sep="\t").fillna("")
...
.panel { padding: 14px 16px 16px; min-height: 350px; }
...
<div class="app">
  <aside class="sidebar">
    ...
    <div class="note">Choose a sample to update read statistics, diversity profiles, and detected taxa tables.</div>
  </aside>
  ...
  <main class="main">
    ...
    <section class="grid">
      ...
      <div class="panel wide">
        ...
        <input type="text" id="taxa-search" class="search-input" placeholder="Filter taxa by name..." />
...
const searchInput = document.getElementById('taxa-search');
...
function renderBrackenTable(rows) {
  const container = document.getElementById('bracken-table-wrap');
  if (!rows || rows.length === 0) {
    container.innerHTML = '<p style="color:var(--muted); padding: 8px;">No taxonomic data found for this sample.</p>';
    return;
  }
  const headers = Object.keys(rows[0]);
  let html = '<table><thead><tr>' + headers.map(h => `<th>${h}</th>`).join('') + '</tr></thead><tbody>';
  rows.forEach(r => {
    html += '<tr>' + headers.map(h => `<td>${typeof r[h] === 'number' ? formatNumber(r[h]) : r[h]}</td>`).join('') + '</tr>';
  });
  html += '</tbody></table>';
...
function updateDashboard(sample) {
  ...
  currentTaxa = DATA.bracken[row.sample] || [];
  searchInput.value = '';
  renderBrackenTable(currentTaxa);
}

searchInput.addEventListener('input', e => {
  const query = e.target.value.toLowerCase();
  const filtered = currentTaxa.filter(row => {
    const taxonName = String(row.name || row.taxonomy_name || '').toLowerCase();
    return taxonName.includes(query);
  });
  renderBrackenTable(filtered);
});
